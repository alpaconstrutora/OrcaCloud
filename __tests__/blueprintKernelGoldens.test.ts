 ...
 *   e `areasPublicas` ({tipo, nome, pontos}). Nenhum dos seis casos abaixo tem
 *   loteamento, e o canônico OMITE as quatro listas quando vazias — então só
 *   a string da versão mudou. Mesma prova, refeita ANTES de tocar num hash:
 *   com a string em 0.57.0 e tudo JÁ no lugar (modelo, os 16 comandos
 *   Add/Set/Move/Delete das quatro famílias, invariantes BAD_BLOCK/BAD_PLOT/
 *   BAD_STREET/BAD_PUBLIC_AREA, canônico ida e volta), os 248 testes de
 *   kernel/goldens passaram sem tocar em nenhum hash; só depois do bump as
 *   falhas apareceram, e todas de hash.
 *
 ...
 *   normalizado até 40 caracteres), só quando declarado. Nenhum dos seis casos
 *   tem. Mesma prova, refeita antes de tocar num hash: com a string em 0.55.0
 *   e tudo JÁ no lugar (modelo, NameSpace/SetSpaceLabelProps aceitando o
 *   campo, invariante `BAD_DEPARTMENT`, canônico ida e volta, cópia de
 *   pavimento-tipo, quadro por setor, sugestão, paleta DEPARTAMENTO), 304
 *   testes de kernel/canônico/goldens/IFC passaram; só depois do bump as seis
 *   falhas foram todas de hash.
 *   0.54.0 → 0.55.0 (21/09/2026): RODAPÉ COMO ELEMENTO (backlog P2 — P2.21) —
 *   família `rodapes: [{level, pontos[], alturaMm, itemCode, descricao,
 *   sugerido?, etiqueta?}]` (trecho de rodapé desenhado; a etiqueta de origem
 *   por índice em `labels`), identidade `rodapes` (prefixo F); a chave só
 *   aparece quando há trecho. Nenhum dos seis casos tem. Mesma prova, refeita
 *   antes de tocar num hash: com a string em 0.54.0 e tudo JÁ no lugar (modelo,
 *   comandos Add/Set/Move/Delete, RemoveLevel, invariantes `BAD_BASEBOARD`,
 *   canônico ida e volta, gerador, quantitativo quant-1.15.0 → 1.16.0 somando
 *   os trechos, orçamento), 400 testes de kernel/canônico/goldens/IFC/
 *   quantitativos/orçamento passaram; só depois do bump as seis falhas foram
 *   todas de hash.
