...
describe('planejarPilares — onde nascem', () => {
  it('3 cantos (o que já tem pilar é pulado), 2 Ts e um intermediário em cada parede de 6 m', () => {
    ...
    // 19 cm numa parede de 15: o pilar entra 20 mm para DENTRO — no canto nos
    // dois eixos, no T através da parede atravessada, no intermediário para o
    // lado do ambiente (a interna w5 tem ambiente dos dois lados: fica no eixo).
    expect(onde).toEqual([
      '20,2000:T',
      '20,3980:CANTO',
      // w1 tem porta em 2700..3500: o meio (3000) cai nela e desvia para a borda
      // esquerda 2700 − 95 − 50 = 2555.
      '2555,20:INTERMEDIARIO',
      '3000,2000:INTERMEDIARIO',
      '3000,3980:INTERMEDIARIO',
      '5980,20:CANTO',
      '5980,2000:T',
      '5980,3980:CANTO',
    ]);
  ...
  it('sem desviar de aberturas, o intermediário cai no meio (3000,0)', () => {
    ...
    expect(em(plano, 3000, 20)?.onde).toBe('INTERMEDIARIO');
    expect(em(plano, 2555, 20)).toBeUndefined();
  });
  ...
  it('o lado maior vai ao longo da parede: horizontal 0°, vertical 90°; 14×40 vira 400 × 140', () => {
    ...
    // 14 cm de profundidade cabe nos 15 da parede: sem empurrão através de w3.
    ...
    // espessura — desempata pela mais LONGA (w5 tem 6 m, w2 tem 4 m) → 0°. Com 0°
    // o lado de 40 cm atravessa w2 (15 cm): empurrado (400 − 150)/2 = 125 para −x.
    const te = plano.pilares.find((p) => p.onde === 'T' && p.at.y === 2000 && p.at.x > 3000)!;
    expect([te.rotacaoDeg, te.at.x]).toEqual([0, 5875]);
    // Canto (6000,4000): w2 vertical 4 m × w3 horizontal 6 m → a mais longa → 0°.
    const canto = plano.pilares.find((p) => p.onde === 'CANTO' && p.at.y > 3900 && p.at.x > 5000)!;
    expect(canto.rotacaoDeg).toBe(0);
  });
  ...
  it('no nó com espessuras diferentes, a parede mais grossa dá o giro', () => {
    ...
    const plano = planejar(m, t);
    // Giro da mais grossa (250, vertical) → 90°; 19 cm cabe nos 25 (sem empurrão em x)
    // e passa dos 15 da horizontal (empurrão de 20 em +y).
    expect(em(plano, 0, 20)!.rotacaoDeg).toBe(90);
  });
  ...
  it('só externas: sem T e nada na interna', () => {
    ...
    const onde = plano.pilares.map((p) => `${p.at.x},${p.at.y}:${p.onde}`);
    expect(onde).toEqual(['20,3980:CANTO', '2555,20:INTERMEDIARIO', '3000,3980:INTERMEDIARIO', '5980,20:CANTO', '5980,3980:CANTO']);
  });
  ...
  it('pilar existente no meio da parede conta como apoio: w1 fica sem intermediário', () => {
    ...
    expect(plano.pilares.filter((p) => p.at.y <= 20 && p.onde === 'INTERMEDIARIO')).toHaveLength(0);
    expect(em(plano, 3000, 3980)?.onde).toBe('INTERMEDIARIO');
  });
  ...
  it('parede fina demais para a seção: aviso de sobressair na linha; 15 cm não avisa', () => {
    ...
    // 25 cm em paredes de 10: empurrado 75 para dentro (−x, +y) e avisado.
    expect(em(plano, 3925, 75)!.aviso).toMatch(/150 mm mais grosso que a parede de 100 mm/);
  ...
  it('a face do pilar coincide com a face externa da parede: nada sobra para fora (canto, T, intermediário)', () => {
    // 16/09/2026, print: "alguns pilares estão ultrapassando os limites das paredes".
    const { m, t } = casa({ existente: false, porta: false });
    const plano = planejar(m, t);
    const canto00 = plano.pilares.find((p) => p.onde === 'CANTO' && p.at.x < 100 && p.at.y < 100)!;
    const anel = pegadaDoPilarPrevisto(canto00);
    // Faces externas das paredes de 15 cm: x = −75 e y = −75. O pilar não passa delas.
    expect(Math.min(...anel.map((c) => c.x))).toBe(-75);
    expect(Math.min(...anel.map((c) => c.y))).toBe(-75);
    const te = plano.pilares.find((p) => p.onde === 'T' && p.at.x < 100)!;
    expect(Math.min(...pegadaDoPilarPrevisto(te).map((c) => c.x))).toBe(-75);
    const inter = plano.pilares.find((p) => p.onde === 'INTERMEDIARIO' && p.at.y < 100)!;
    expect(Math.min(...pegadaDoPilarPrevisto(inter).map((c) => c.y))).toBe(-75);
    // Pilar mais fino que a parede (14 cm em 15) fica centrado no eixo.
    const fino = planejar(m, t, { larguraMm: 140, profundidadeMm: 140 });
    expect(fino.pilares.find((p) => p.onde === 'CANTO' && p.at.x < 100 && p.at.y < 100)!.at).toEqual({ x: 0, y: 0 });
  });
...
describe('planejarPilares — o lote', () => {
  it('ids previstos batem com os que o kernel dá; AddStructural antes; uma parede cede uma vez', () => {
    ...
    expect(p4).toMatchObject({ kind: 'PILAR', pontos: [{ x: 20, y: 2000 }], larguraMm: 190, profundidadeMm: 190, alturaMm: 2800, baseMm: 0 });
  });
