        ...
        if (order.paymentTermType === 'Parcelado') {
            ...
            return (
                <div className="space-y-3">
                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Cronograma de Parcelas</h4>
                            ...
                            return (
                                <div key={i} className="px-5 py-4 flex items-center justify-between hover:bg-white transition-colors">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-white border border-gray-100 flex items-center justify-center text-xs font-black text-gray-900">
                                            {i + 1}
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-gray-900">Parcela {i + 1}/{count}</p>
                                            <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Vencimento: {dueDate.toLocaleDateString('pt-BR')}</p>
                                        </div>
                                    </div>
            ...
            return (
                <div className="space-y-3">
                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Condição À Vista</h4>
                    <div className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-emerald-600 font-bold uppercase tracking-widest">Previsão: {dueDate.toLocaleDateString('pt-BR')}</p>
                            </div>
                        </div>
        ...
        <div className="flex flex-col h-full bg-white rounded-[2.5rem] overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                                ...
                                <span className={`px-2 py-1 rounded-lg text-xs font-black uppercase tracking-wider ${order.status === 'Confirmado' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                                    {order.status}
                                </span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    ...
                    <div className={`p-8 rounded-[2rem] border transition-all duration-500 flex flex-col justify-between ${isApproved ? 'bg-emerald-50/30 border-emerald-100 shadow-emerald-50/50' : 'bg-rose-50/30 border-rose-100 shadow-rose-50/50'}`}>
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className={`text-xs font-black uppercase tracking-widest mb-1 ${isApproved ? 'text-emerald-600' : 'text-rose-600'}`}>Status de Aprovação</h3>
                        ...
                        {['Recebido', 'Divergência'].includes(order.status) || isApproved ? (
                            <button
                                ...
                                className={`mt-6 w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg ${isApproved ? 'bg-rose-500 text-white shadow-rose-200 hover:bg-rose-600' : 'bg-emerald-600 text-white shadow-emerald-200 hover:bg-emerald-700'}`}
                            >
                            ...
                            <div className="mt-6 space-y-3">
                                <div className="p-4 bg-white/50 rounded-2xl border border-rose-100 flex items-start gap-3">
                                    ...
                                    <p className="text-xs font-bold text-rose-700 uppercase leading-relaxed text-left">
                                        Aguardando confirmação de recebimento pelo setor de suprimentos para permitir aprovação.
                                    </p>
                                </div>
                                <button
                                    disabled
                                    className="w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest bg-gray-100 text-gray-400 cursor-not-allowed"
                                >
                                    APROVAÇÃO BLOQUEADA
                                </button>
                            </div>
            ...
            <div className="px-10 py-8 border-t border-gray-100 bg-gray-50 flex items-center justify-end gap-4">
                ...
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex items-center gap-3 px-10 py-4 bg-black text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-gray-200 disabled:opacity-50"
                >
