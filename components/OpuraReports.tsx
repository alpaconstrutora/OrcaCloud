        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                compareRows.length === 0 ? (
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {compareRows.map((r, i) => (
                                    <tr key={r.dimension_key ?? `c-${i}`} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                                        <td className="px-5 py-2.5 text-sm font-normal text-gray-700 truncate max-w-[280px]">{r.dimension_label}</td>
                                        <td className="px-3 py-2.5 text-right tabular-nums text-sm font-medium text-gray-900">{fBRLshort(r.valorA)}</td>
                                        <td className="px-3 py-2.5 text-right tabular-nums text-sm font-normal text-gray-500">{fBRLshort(r.valorB)}</td>
                                        <td className={`px-3 py-2.5 text-right tabular-nums text-sm font-medium ${r.delta < 0 ? 'text-red-600' : r.delta > 0 ? 'text-emerald-600' : 'text-gray-300'}`}>
                                            {r.delta > 0 ? '+' : ''}{fBRLshort(r.delta)}
                                        </td>
                                        <td className="px-5 py-2.5 text-right tabular-nums">
                                            {r.variacao === null ? (
                                                <span className="text-indigo-400 text-xs font-normal">novo</span>
                                            ) : (
                                                <span className={`text-sm font-medium inline-flex items-center gap-0.5 ${r.variacao >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                            ...
                            <tfoot>
                                <tr className="border-t-2 border-gray-100 text-sm font-medium text-gray-900">
                ...
                <div className="text-center py-12 bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <p className="text-sm text-gray-500">Ajuste os filtros ou o período selecionado.</p>
                </div>
            ) : (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <table className="w-full text-sm">
                                ...
                                <tr key={r.dimension_key ?? `row-${i}`} onClick={() => openDrill(r)}
                                    ...
                                    <td className="px-5 py-2.5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-normal text-gray-700 truncate group-hover:text-blue-700">{r.dimension_label}</span>
                                            <ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-blue-400 flex-shrink-0 ml-auto" />
                                        </div>
                                    </td>
                                    <td className="px-3 py-2.5 text-right tabular-nums text-sm font-normal text-gray-500">{r.qtd}</td>
                                    <td className={`px-3 py-2.5 text-right tabular-nums text-sm font-medium ${r.realizado < 0 ? 'text-red-600' : 'text-gray-900'}`}>
                                        {fBRLshort(r.realizado)}
                                    </td>
                                    <td className="px-3 py-2.5 text-right tabular-nums text-sm font-normal text-gray-500">{fBRLshort(r.previsto)}</td>
                                    <td className={`px-5 py-2.5 text-right tabular-nums text-sm font-medium ${r.vencido > 0 ? 'text-amber-600' : 'text-gray-300'}`}>
                            ...
                            <tbody>
                                {entries.map(e => (
                                    <tr key={e.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                                        <td className="px-4 py-2.5 whitespace-nowrap text-sm font-normal text-gray-500 tabular-nums">
                                            {e.transaction_date.split('-').reverse().join('/')}
                                        </td>
                                        <td className="px-2 py-2.5">
                                            <p className="text-sm font-normal text-gray-700 truncate max-w-[260px]">
