...
const PlanoContasImportModal: React.FC<Props> = ({
    organizationId,
    existingItems,
    onClose,
    onSuccess,
    ...
    const handleImport = async () => {
        ...
        setImporting(true);
        try {
            const res = await financialRegistryService.upsertPlanoContas(organizationId, toImport);
                                    ...
                                    {rows.map(row => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-2.5">
                                                    <span className="text-sm font-normal text-gray-800">{row.name || <span className="text-red-400 italic">vazio</span>}</span>
                                                    {row.errorMsg && (
                                                        <p className="text-xs text-red-500 font-normal mt-0.5">{row.errorMsg}</p>
                                                    )}
                                                </td>
                                                <td className="px-4 py-2.5 text-center">
                                                    <span className={`inline-flex items-center gap-1 text-sm font-normal ${cfg.color}`}>
...
export default PlanoContasImportModal;
