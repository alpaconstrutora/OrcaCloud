...
- ~~Dois escritores em `projects.budget`~~ — **unificado em 06/09**. O da Medição criava a linha com `crypto.randomUUID()` e código `MED-<aleatório>`; a única coisa que impedia duplicata ao reexportar era casar pelo TEXTO da descrição, então renomear o item na biblioteca fazia o orçamento crescer sozinho. Agora o id é determinístico (`med:<itemDeBiblioteca>`), como o `bp:` da Planta, e a regra saiu de um componente de 1.400 linhas para `utils/medicaoNoOrcamento.ts` (10 casos).
  ⚠️ Linhas JÁ exportadas mantêm o id aleatório — de propósito. O id de uma linha de orçamento é o id da TAREFA no cronograma; reescrevê-lo romperia o vínculo com o planejamento em silêncio. Elas continuam sendo achadas pela descrição.
- ~~`Building3DViewer` (Planta AI) nunca auditado~~ — **auditado em 06/09, e TINHA irmão**: o mesmo defeito de câmera (`<Canvas camera>` só vale na montagem, e "Centralizar" chamava `controls.reset()`, devolvendo o enquadramento de ANTES da mudança de cenário). Corrigido. Duas cenas com o mesmo erro fizeram a conta sair para `utils/camera3d.ts`, que agora serve as duas.
  ✅ **E agora tem prova de runtime**: `docs/spikes/planta-ai-3d/` (06/09), espelhando o da Planta Inteligente. Três portões, e o terceiro é o que teste nenhum alcança — o prédio CRESCE com a cena montada e tem de continuar enquadrado, que é o gesto exato que expunha o defeito.
  ⚠️ **E o portão certo não era o óbvio.** O primeiro que escrevi foi "fração mínima de pixel pintado", como no harness do blueprint. Medido: corrigido rende **5,9% pintado e 0 px na borda superior**; com o defeito, **31,8% e 29.410 px**. Com a câmera parada o prédio TRANSBORDA — pinta mais, não menos. O piso de fração aprovaria o defeito e reprovaria a correção. O sinal é **encostar na borda**.
