...
### P2.12 — Paredes curvas: arco discretizado com metadado (20/09/2026) · backlog P2 · kernel 0.47.0 → 0.48.0

**O que entrou.** A parede curva NÃO é entidade nova: é uma corrente de paredes retas curtas (facetas) com o metadado `Wall.arco = {centro, raioMm}` do círculo em que as duas pontas estão. Tudo o que o kernel já faz com parede reta (arranjo planar, ambiente, área, abertura, quantitativo, orçamento, IFC, 3D) vale sem uma linha nova; o metadado serve para o canvas desenhar a curva de verdade e para o painel reconhecer o arco.

- **Kernel** (`utils/blueprintKernel/arco.ts`): `circuloPorTresPontos`, `varreduraDoArco` (o sentido que passa pelo 3º ponto), `facetasDoArco` (passo ≤ 15° E flecha ≤ 10 mm, teto 90 facetas), `discretizarArco` (vértices em mm inteiros, pontas exatas), `arcoConsistente` (pontas a ≤ 3 mm do círculo), `segmentosDoMesmoArco`. Comando **`AddCurvedWall`** {a, b, passandoPor, espessura, altura, alinhamento, camadas} → N `walls` com o arco; `DEGENERATE_ARC` para colineares. Invariante `BAD_ARC` (raio inteiro > 0, centro inteiro). Translação/espelho/giro/duplicar/grupo/pavimento tipo levam o centro junto; **`retirarArcosDesfeitos`** roda em todo comando e tira o metadado da faceta cuja ponta saiu do círculo (MoveVertex, ponta esticada, split na corda) — a parede vira reta e honesta, nunca um arco que não passa pelas pontas.
- **Canônico**: `arco` só na faceta (parede reta não ganha chave) — **bump 0.48.0**, ritual dos goldens no cabeçalho de `blueprintKernelGoldens.test.ts` (314 testes com a string velha; depois só os 6 hashes).
- **UI**: item "Parede curva" no menu Componentes › Alvenaria (ferramenta `parede-curva`: três cliques — início, fim, ponto por onde passa; Shift trava o 2º; prévia com corda, arco facetado, "R 2,56 m · 17 facetas"); canvas pinta a faceta como **arco** (`ctx.arc` pelo centro em tela) nas passadas de silhueta e miolo, com a emenda entre facetas do mesmo arco atravessada pelo miolo (sem risco radial); painel da parede: "Faceta de parede curva · raio X m · N facetas" + **Selecionar o arco** (seleciona as N).

**Decisões.** (1) Sem id de arco: facetas se reconhecem pelo mesmo (centro, raio) no pavimento — um id sorteado entraria no hash e duas sessões desenhando a mesma curva divergiriam. (2) Metadado descartável, não invariante: um payload antigo ou um vértice movido nunca derrubam o modelo; a consistência é restaurada por subtração. (3) Camadas (faixas de composição) não são pintadas na faceta curva — a corda sairia fora do arco; a composição continua valendo no quantitativo. (4) Paredes **inclinadas** ficam fora (exigem face não vertical no 3D e no IFC — corte diferente).

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 15): menu → "Parede curva" (botão passa a dizer "Parede curva"); três cliques → "paredes 14 → 31 (17 facetas)"; canvas com o arco contínuo, sem riscos nas emendas (captura); Selecionar + clique na curva → painel "Faceta de parede curva · raio 2,56 m · 17 facetas"; "Selecionar o arco" → "17 paredes · 7,62 m de parede"; 0 erros de página. Testes: `blueprintParedeCurva.test.ts` (3: círculo/facetas/flecha/sentido; D com meia-volta fecha ambiente de ~37,9 m² e canônico ida e volta com 20 facetas; translação/espelho/giro levam o centro e MoveVertex tira o arco só da faceta movida), editor P2.12 (menu, painel, seleção do arco), goldens recapturadas; suíte 412 arquivos / 4898 testes; tsc, check-ui e build OK; `kernel.bundle.mjs` regenerado e `planta-api` v7 publicada (`plantaApi.test.ts` 3/3).

### P2.13 — Cobertura por extrusão (20/09/2026) · backlog P2 · kernel 0.48.0 → 0.49.0

**O que entrou.** O "telhado por extrusão" do Revit: um PERFIL em corte extrudado ao longo de um eixo em planta. A cobertura continua sendo o que sempre foi — águas planas (`Agua`) — e a extrusão é um **gerador**: cada trecho reto do perfil vira uma água retangular com o beiral no lado baixo, a inclinação do trecho e a base na cota baixa.

- **Kernel** (`utils/blueprintKernel/coberturaExtrusao.ts`): `perfilDeCobertura` com quatro perfis prontos — **duas águas**, **uma água (shed)**, **abóbada** (arco pelo beiral e pela flecha, em número PAR de facetas para o topo ser vértice; passo/flecha de `arco.ts`) e **dente de serra** (N dentes) —, `aguasDaExtrusao(eixoA, eixoB, perfil)` (s através do eixo, centrado nele; z sobre o piso; anel de 4 pontos por trecho; `beiralIndex` 3 ou 1 conforme o lado baixo), erros nomeados (`PERFIL_VOLTA`, `PERFIL_INGREME` > 300 %, `EIXO_DEGENERADO`, `PERFIL_SEM_AGUA`). **Face vertical não vira água**: o "vidro" do dente de serra é pulado e contado — fecha-se com parede ou cortina. Comando **`AddRoofByExtrusion`** grava as águas com `Agua.extrusao = {a, b}` (o eixo), metadado de agrupamento que anda com translação/espelho/giro/duplicação e sai da água cujo vértice for movido. Invariante `BAD_EXTRUSION`.
- **Canônico**: `extrusao` só na água nascida do gesto (água à mão não ganha chave) — **bump 0.49.0**, ritual dos goldens no cabeçalho (374 testes com a string velha; depois só os 6 hashes).
- **UI**: item "Cobertura por extrusão" no menu Componentes › Cobertura (ferramenta `cobertura-extrusao`, dois cliques no eixo, Shift trava; prévia com o eixo e a faixa do vão centrada, "L × vão"); barra de opções `CamposDaExtrusao` (perfil, vão, beiral a, cumeeira a / lado alto a / flecha, dentes, espessura; persistida em `blueprint:cobertura-extrusao`); painel da água: "Água de cobertura por extrusão · N águas do mesmo eixo" + **Selecionar a cobertura**. 3D e IFC seguem por água — a abóbada aparece facetada sem uma linha nova.

**Decisões.** (1) Gerador, não entidade: quantitativo (`medirAgua`), orçamento, IFC (`IfcRoof` por água) e 3D não sabem que houve extrusão — o mesmo caminho da parede curva. (2) Perfil centrado no eixo (s = vão/2 sobre A→B): o eixo é a cumeeira das duas águas e o meio da abóbada, que é onde quem desenha mira. (3) Face vertical pulada em vez de recusada: o dente de serra é legítimo e o fechamento do shed é outra família.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 15): menu → "Cobertura por extrusão"; barra com perfil **Abóbada**, vão 6 m, flecha 1,5 m; dois cliques no eixo → 14 águas criadas e selecionadas; painel "Água de cobertura por extrusão · 14 águas do mesmo eixo" e "3,14 m² de telha" por faceta; "Selecionar a cobertura" → "14 selecionados"; Vista 3D com a abóbada facetada ao lado do bloco (captura); 0 erros de página. Testes: `blueprintCoberturaExtrusao.test.ts` (3: os quatro perfis; duas águas → dois retângulos com beiral no lado baixo, cumeeira a 4,20 m sobre o eixo, 40 m² em planta cada; dente de serra 3 águas + 2 faces puladas; erros de perfil; comando, canônico ida e volta, translação leva o eixo, vértice movido sai do grupo, `BAD_EXTRUSION`), editor P2.13 (menu, barra troca cumeeira↔flecha↔dentes, painel e seleção), goldens recapturadas; suíte 413 arquivos / 4902 testes; tsc, check-ui e build OK; bundle regenerado e `planta-api` publicada (`plantaApi.test.ts` 3/3).

### P2.14 — Planta de forro (20/09/2026) · backlog P2 · sem bump

**O que entrou.** A "reflected ceiling plan" como quarta **vista de planta** (E0.3): `forro`, do pavimento **atual** (`nivel: 'ATUAL'` — o editor resolve), read-only como as outras.

- **`utils/blueprintPlantaDeForro.ts`** (derivação pura): `forrosDoNivel` — por ambiente, material (a camada de acabamento do forro declarado na E7.2), rebaixo e **pé-direito útil** (pavimento − rebaixo, o mesmo número do quantitativo P2.8), e o rótulo em três linhas ("Sala / Forro: gesso acartonado / rebaixo 0,30 m · PD 2,70 m"; sem forro: "Laje aparente (sem forro declarado) / PD 3,00 m"); `idsOcultosNaPlantaDeForro` — fica o que está no teto (luminária de teto, interruptor, eletroduto com as duas cotas ≥ 1,80 m, tudo da MECÂNICA, componentes de CLIMATIZAÇÃO, paredes, vãos, estrutura, núcleos), some o que está no chão (tomadas e demais pontos elétricos, hidráulica, quadros, mobiliário, escadas, vagas, guarda-corpos, cortes); `resumoDaPlantaDeForro` (ambientes com forro, luminárias, difusores).
- **Vista**: `VISTAS_DE_PLANTA += 'forro'`, `AJUSTE_DA_VISTA.forro`, `idsOcultosNaVista` delega; `SeletorDeVista` ganha "Planta de forro" (ícone lâmpada) — 10 vistas fixas.
- **Editor/canvas**: na vista, o rótulo do ambiente diz o forro no lugar de área/perímetro (mesmo mecanismo `rotulosDeAmbiente`); ambientes com forro declarado recebem a **hachura de placas de 600 mm** (`ambientesComForro`, recorte por anel com furos); a faixa da vista traz o resumo.

**Decisões.** (1) Vista, não desenho: nada é gravado, nenhum quantitativo muda — é o mesmo caminho de situação/implantação/cobertura. (2) Interruptor fica: a planta de forro é onde a iluminação se lê com seus comandos. (3) Eletroduto "de teto" = as duas cotas ≥ 1,80 m; a descida à tomada some com a tomada. (4) A prancha exportada continua sendo a planta cheia (as vistas de planta da E0.3 já não filtram a exportação) — a planta de forro em PDF/DXF fica registrada como pendência de exportação.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 14): "Vista: Planta de forro" → faixa "Planta de forro · Térreo — O pavimento visto de baixo… · 4 de 4 ambiente(s) com forro declarado · 4 luminária(s) de teto · 0 difusor(es)/grelha(s)"; canvas com a malha de forro nos quatro ambientes, luminárias, interruptores e eletrodutos altos, sem mobiliário, tomadas e hidráulica (captura); "Vista: Planta" devolve tudo; 0 erros de página. Testes: `blueprintPlantaDeForro.test.ts` (2: rótulo/material/rebaixo/PD útil e resumo; oculta chão e mantém teto, `forro` na lista com nível ATUAL e delegação, situação segue escondendo tudo), editor P2.14 (vista read-only com faixa e resumo), contagem de vistas fixas 9 → 10 no teste do ribbon; suíte 414 arquivos / 4905 testes; tsc, check-ui e build OK.

### P2.15 — Nuvens de revisão (21/09/2026) · backlog P2 · kernel 0.49.0 → 0.50.0

**O que entrou.** A nuvem de revisão do desenho técnico: um contorno recortado em torno do que mudou, com o triângulo "Δn" da revisão, e a **tabela de revisões no carimbo** da prancha.

- **Kernel**: tipo de anotação **`NUVEM`** (E8.1 ganha o sexto tipo; ≥ 3 pontos) com **`revisao: {numero, data}`** — número inteiro ≥ 1, data ISO `AAAA-MM-DD` informada por quem cria (o kernel não sabe que dia é); toda nuvem tem revisão e nenhuma outra anotação a leva (invariantes `BAD_ANNOTATION`); o `texto` é a descrição da alteração. `AddAnotacao`/`SetAnotacaoProps` aceitam `revisao`. Canônico emite `revisao` só na nuvem — **bump 0.50.0**, ritual dos goldens no cabeçalho (347 testes com a string velha; depois só os 6 hashes).
- **Geometria** (`blueprintAnotacoes.ts`): `contornoDaNuvem` — cada lado do polígono vira uma fila de meias-luas para FORA (corda ≈ 2,5 × altura do texto, mín. 150 mm), a mesma polilinha para tela, PDF e DXF; `posicaoDaEtiquetaDaNuvem` (o vértice mais alto); `revisoesDasAnotacoes`/`revisoesDoModelo` (uma linha por número: data mais recente, descrições sem repetir, nº de nuvens); `proximaRevisao`; `dataDaRevisaoBr`. A seleção pega a nuvem pelo polígono, como a hachura.
- **UI**: botão **Nuvem de revisão** em Inserir › Anotações (conta as nuvens); barra de opções com "Revisão nº" (nasce na próxima livre; lista "Δ1 (2) · Δ2 (1)" do desenho); painel da anotação com número, data e descrição; canvas desenha o contorno recortado, o triângulo com o número e a descrição ao lado.
- **Prancha**: PDF desenha a nuvem igual à tela e o **carimbo ganha a tabela "Rev. · Data · Descrição"** (as 4 mais recentes, do mais novo ao mais velho) quando há nuvem na vista — planta e corte/elevação; DXF leva a polilinha e o texto "Δn descrição" na camada de anotações.

**Decisões.** (1) Nuvem é anotação, não entidade: entra no fluxo que já existe (vista, seleção, painel, PDF, DXF, versões) e some com o pavimento/corte como as outras. (2) A revisão fica NA nuvem, não numa tabela à parte: a tabela do carimbo é derivada (uma linha por número) e nunca diverge das nuvens que estão no desenho. (3) A data é dado, não `hoje()` do kernel — determinismo do payload e do hash.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 14): Inserir › "Nuvem de revisão" → barra "Revisão nº 1 · primeira revisão do desenho"; 4 cliques + duplo clique em volta de um cômodo → painel "Nuvem de revisão · na planta · 4 ponto(s)" com Revisão nº 1 e data 21/09/2026; descrição digitada → canvas com o contorno recortado, o triângulo "1" e "Porta da sala deslocada 30 cm" (captura); botão passa a "Nuvem de revisão 1"; 0 erros de página. Testes: `blueprintNuvemDeRevisao.test.ts` (3: contorno para fora nos dois sentidos, etiqueta no topo; comando exige revisão, canônico ida e volta, tabela de revisões, próxima revisão, invariantes; PDF com "Δ2"/descrição/tabela "Rev." no carimbo e DXF), painel da nuvem em `blueprintAnotacaoCanvas.test.tsx`, `blueprintAnotacoes` (resumo com NUVEM), goldens recapturadas; suíte 415 arquivos / 4909 testes; tsc, check-ui e build OK; bundle regenerado e `planta-api` publicada (`plantaApi.test.ts` 3/3).

### P2.16 — Tabelas personalizadas (21/09/2026) · backlog P2 · sem bump · migração 000060

**O que entrou.** Os "schedules" do Revit — uma tabela definida pelo usuário sobre uma **família** de peças (paredes, esquadrias/vãos, estrutura, águas, escadas, trechos, pontos, quadros), com as colunas que ele escolher, filtro, agrupamento, ordenação e totais — em Analisar › **Tabelas**.

- **Sem vocabulário novo**: as colunas são as **variáveis que as fórmulas de parâmetro já enxergam** (`variaveisDaPeca`: nativas, do pavimento, parâmetros gravados e calculados por fórmula) e o filtro é uma expressão da **mesma linguagem** das fórmulas e das regras (`avaliar`; ex.: `tipo == "porta" e largura >= 0.8`). `utils/blueprintTabelas.ts`: `definicaoDaColuna` (JSONB sanitizado, chaves válidas, ≤ 16 colunas), `validarDefinicao`, `colunasDisponiveis` (nativas + pavimento + parâmetros da org e os gravados no modelo), **`montarTabela`** (filtro por peça com erro reportado uma vez; grupos por chave; ordenação; totais SOMA/MÉDIA/CONTAGEM/MÍN/MÁX por grupo e geral; coluna que a peça não tem = célula vazia), `tabelaParaPlanilha`, `celula`, **4 sementes** (quadro de esquadrias, paredes por pavimento, pontos elétricos com potência, pilares e vigas).
- **Persistência**: `blueprint_table_definitions` (org, nome único, `definicao` JSONB, active; RLS `is_org_member`) — migração `aplicar_20270921000060` aplicada; `services/blueprintTabelaService.ts` (list/create/update/remove por soft delete).
- **Tela** `TelaTabelas.tsx`: lista das tabelas da organização (semear as que faltam, nova, editar, excluir com confirmação); editor da definição (nome, família, filtro, agrupar/ordenar/ordem, colunas por chips com rótulo e total por coluna) que **monta ao vivo** enquanto se edita; a tabela montada em `StandardTable` por grupo com subtotal e total geral, clique na linha seleciona a peça; **Exportar .xlsx** (`artefatoDeTabelaXlsx`). Ribbon Analisar › Tabelas com a contagem.

**Decisões.** (1) Definição na organização, tabela derivada do modelo aberto — nada no payload nem no hash. (2) Reaproveitar a linguagem das fórmulas em vez de um construtor de filtros: uma sintaxe só para regra, fórmula e tabela. (3) Grupo como uma `StandardTable` por grupo com linha de subtotal — o padrão de tabela do app sem um componente novo. (4) Coluna ausente é célula vazia, não erro: a mesma tabela serve a modelos com e sem o parâmetro.

**Prova.** *No app real* (estudo "Planta 14/09/2026"): com escritas bloqueadas (14) — Analisar › Tabelas, "Nova tabela" › Esquadrias com tipo/largura/altura/area/pavimento.nome, agrupar por tipo, média da largura e soma da área → "12 de 12 peça(s) · 2 grupo(s) por tipo", subtotais (janela: área 4,32 m²; porta: 15,12 m²) e total geral 19,44 m²; filtro `tipo == "porta" e largura >= 0.8` → 8 de 12; download `tabela-quadro-de-esquadrias-prova.xlsx`. *Com escrita real* autorizada ("Semear na Alpa e manter"), allow-list só em `blueprint_table_definitions`: "Semear 4 pronta(s)" → modal REGRA #5 → Alpa → 4 POST; a ativa monta "14 de 14 peça(s) · Paredes · 2 grupo(s) por pavimento.nome"; editar "Pilares e vigas" (ordem ASC) → 1 PATCH, "Tabela atualizada"; **banco**: 4 linhas ativas na Alpa (wall 5 colunas DESC, structural 6 ASC, terminal 4 DESC, opening 6 ASC); 0 erros de página; escritas bloqueadas 0 (só a allow-list passou). Testes: `blueprintTabelas.test.ts` (2: sanitização/validação/colunas disponíveis; montagem com filtro, grupos, ordem, totais, célula vazia, erro de filtro, planilha, sementes), editor P2.16 (tela em fluxo, tabela salva montada com totais, nova tabela ao vivo e gravação pelo serviço com dublê); suíte 416 arquivos / 4912 testes; tsc, check-ui e build OK.

### P2.17 — Vista dependente (21/09/2026) · backlog P2 · kernel 0.50.0 → 0.51.0

**O que entrou.** A "dependent view" do Revit: um **recorte nomeado de uma planta, com escala própria** — o mesmo modelo visto por uma janela, para partir uma planta grande em pranchas (linhas de encontro) e para ampliações que não são de um ambiente só.

- **Kernel**: família **`vistasDependentes`** (`VistaDependente {levelId, nome ≤ 60, recorte {minX,minY,maxX,maxY} em mm, denominador ≥ 1}`), comandos `AddVistaDependente` (recorte normalizado min/max), `SetVistaDependenteProps`, `DeleteVistaDependente`; `RemoveLevel` leva as vistas do pavimento; invariantes `BAD_DEPENDENT_VIEW`; identidade `vistasDependentes` (prefixo **D**). Canônico emite a chave só quando há vista — **bump 0.51.0**, ritual dos goldens no cabeçalho (319 testes com a string velha; depois só os 6 hashes). Vive no payload porque é decisão do desenhista, como as anotações.
- **Editor**: `VistaBlueprint` ganha `dependente:<id>`; o seletor de vista lista os recortes (ícone de recorte) depois dos cortes; abrir um **ativa o pavimento dele e enquadra o recorte**; a vista NÃO conta como "em vista" — abas, ferramentas e painéis são os da planta (continua editável). Faixa própria: pavimento, tamanho do recorte, **Nome** e **Escala** editáveis, **Redefinir recorte** (arraste), **Excluir** (confirmação) e "voltar à planta". Botão **Vista dependente** em Arquitetura › Vistas (conta as do pavimento) arma o arraste do recorte — o mesmo gesto da região de geração — e cria "Vista N" a 1:50, já aberta.
- **Canvas**: fora do recorte um véu (`evenodd`), moldura e "nome · 1:escala"; na planta-mãe, **linhas de encontro** tracejadas com o nome e a escala de cada vista do pavimento; ENQUADRAR enquadra o recorte.
- **Pranchas**: `planejarConjunto` gera uma prancha **AMPLIAÇÃO por vista dependente** ("nome (pavimento)") com o recorte e a escala dela (`vistaDependenteId`), quando o template inclui ampliações — sai no PDF pelo mesmo caminho da ampliação de ambiente.

**Decisões.** (1) Editável por dentro: a vista dependente é a planta com uma janela, não uma vista de leitura (situação/implantação são). (2) Recorte normalizado no comando (min/max em ordem) — o arraste pode vir em qualquer sentido. (3) Sem geometria própria: o que se vê no recorte é o pavimento — anotações, peças e cotas inclusive; a prancha herda tudo.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 15): Arquitetura › "Vista dependente" → arraste sobre a metade esquerda da casa → faixa "Vista dependente · Térreo · recorte 8,27 × 8,13 m · o desenho continua editável…" e canvas com o recorte nítido, o resto esmaecido, "Vista 1 · 1:50" na moldura e a barra de ferramentas ainda em "Clique para iniciar a parede" (captura); nome → "Ala esquerda" e escala 1:25 pela faixa; o seletor passa a dizer "Ala esquerda"; "voltar à planta" → linhas de encontro na planta-mãe; botão "Vista dependente 1"; 0 erros de página. Testes: `blueprintVistaDependente.test.ts` (2: Add/Set/Delete, normalização, invariantes, RemoveLevel; canônico ida e volta e as pranchas por vista), editor P2.17 (seletor, faixa editável, contagem, excluir volta à planta), goldens recapturadas; suíte 417 arquivos / 4915 testes; tsc, check-ui e build OK; bundle regenerado e `planta-api` publicada (`plantaApi.test.ts` 3/3).

### P2.18 — Famílias aninhadas: conjuntos de componentes (21/09/2026) · backlog P2 · kernel 0.51.0 → 0.52.0

**O que entrou.** A família aninhada do Revit: um **CONJUNTO** é um componente-pai cujos filhos são componentes comuns com `paiUid`. Cada filho continua sendo a peça que é (o vaso conta no quantitativo, liga no ponto hidráulico, sai no IFC); o pai é o que se pega para mover, girar e apagar tudo de uma vez.

- **Kernel**: tipos `CONJUNTO_BANHEIRO` (vaso, lavatório, box), `CONJUNTO_JANTAR` (mesa + 4 cadeiras), `CONJUNTO_DORMITORIO` (cama + 2 criados) e `CONJUNTO_COZINHA` (bancada, fogão, geladeira) no catálogo (símbolo `CONJUNTO`); `CONJUNTOS_DE_COMPONENTES` com os filhos (deslocamento no referencial do pai sem giro + rotação relativa); `Componente.paiUid`; comando **`AddConjunto`** (pai centrado na caixa envolvente dos filhos — `extensaoDoConjunto` — e filhos girados junto); cascata em `MoveComponente` (vetor), `SetComponenteProps.rotacaoGraus` (giro dos filhos em torno do pai), `DeleteComponente` (pai leva os filhos; filho sai sozinho) e `TranslateEntities` (filhos dos conjuntos selecionados, uma vez só); filho cujo pai sumiu fica solto; invariantes `BAD_COMPONENT` (pai existe, é conjunto, mesmo pavimento, não é ele mesmo; conjunto não é filho de conjunto). Canônico: `pai` por **índice** na lista canônica (uid fora do hash), só nos filhos — **bump 0.52.0**, ritual dos goldens no cabeçalho (332 testes com a string velha; depois só os 6 hashes).
- **UI**: menu Mobiliário › "Mobiliário — conjuntos" (4 itens); um clique insere pai + filhos e seleciona o pai; canvas desenha o pai como contorno tracejado fino sem preenchimento e o clique prefere a PEÇA ao conjunto que a envolve; painel do componente: pai → "Conjunto com N peça(s) — mover, girar e excluir levam todas · Selecionar as peças"; filho → "Faz parte do conjunto X · Selecionar o conjunto". **3D e IFC** pulam o pai (é agrupamento, não objeto).

**Decisões.** (1) Filhos são componentes de verdade, não geometria embutida: tudo o que já existe para componente (ponto hidráulico, quantitativo, IFC, fases, tipos) vale para eles sem uma linha nova. (2) Vínculo pelo uid do pai (índice no canônico): apagar o pai não deixa fantasma; apagar um filho não desmonta o conjunto. (3) Um nível só (conjunto dentro de conjunto é recusado) — é o que cabe na semântica "mover tudo de uma vez" sem árvore.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 15): Mobiliário › "Conjunto de jantar" → um clique fora do lote → painel "Conjunto com 5 peça(s) — mover, girar e excluir levam todas"; giro 90° pelo painel → as quatro cadeiras giram em torno da mesa (captura); 0 erros de página. Testes: `blueprintFamiliasAninhadas.test.ts` (3: catálogo; inserir/mover/girar/apagar com cascata, translação em lote sem dobrar, filho sozinho, invariantes; canônico ida e volta com o pai por índice e giro dos filhos), editor P2.18 (menu, painel do pai e do filho, seleção das peças/do conjunto, excluir o pai apaga tudo), goldens recapturadas; suíte 418 arquivos / 4919 testes; tsc, check-ui e build OK; bundle regenerado e `planta-api` publicada (`plantaApi.test.ts` 3/3).

### P2.19 — Sub-regiões do terreno (21/09/2026) · backlog P2 · kernel 0.52.0 → 0.53.0

**O que entrou.** A "subregion" da toposuperfície: um polígono do lote com **material de superfície** — grama, jardim, brita, piso drenante (permeáveis); intertravado, concreto, deck, asfalto, espelho d'água (impermeáveis). Três respostas que o desenho não dava: a **taxa de permeabilidade desenhada** (a zona já tinha `permeabilidade_min` sem nada para comparar), as **áreas de paisagismo/pavimentação externa** para o orçamento, e a implantação com o chão diferenciado. Os taludes "avançados" (por aresta, banquetas, muros, drenagem) já estavam na E3/E4 da topografia — esta fase fecha a parte que faltava do item.

- **Kernel**: família **`subRegioes`** (`SubRegiao {levelId, material, pontos ≥ 3, nome}`), `MATERIAIS_DE_SUB_REGIAO` (9) com `FICHA_DO_MATERIAL_DE_SUB_REGIAO` (rótulo, permeável, cor, trama); comandos `AddSubRegiao`, `SetSubRegiaoProps`, `MoveSubRegiaoVertex`, `DeleteSubRegiao`; `RemoveLevel` leva junto; invariantes `BAD_SUBREGION`; identidade `subRegioes` (prefixo **J**). Canônico emite a chave só quando há sub-região — **bump 0.53.0**, ritual dos goldens no cabeçalho (361 testes com a string velha; depois só os 6 hashes).
- **`utils/blueprintSubRegioes.ts`**: `medirSubRegioes`, `quadroDeSubRegioes` (por material, permeável × impermeável, taxa e cobertura contra a área do lote), `variaveisDePermeabilidade` (só quando há sub-região — sem ela não se afirma permeabilidade zero), `avisosDeSobreposicao`.
- **Regras**: o LOTE ganha `area_permeavel`, `area_impermeavel`, `taxa_permeabilidade`; semente **`sem-lote-permeabilidade`** (`taxa_permeabilidade >= permeabilidade_min`, quando a zona tem mínimo) — Legislação passa a 33 sementes.
- **Orçamento**: escopo `TERRENO` e medida **`AREA_SUBREGIAO`** (uma linha por sub-região, m², filtro por material ou nome), alimentada pelos `extras` da geração como a armadura.
- **UI**: ferramenta **Sub-região** em Terreno › Lote (cliques nos vértices, fecha no 1º; material na barra, persistido); canvas pinta cor + trama do material (pontos, grade, diagonal, ondas), nome e área; clique seleciona; `PainelSubRegiaoSelecionada` (material, nome, área, permeável, excluir); **Dados do lote** ganha o quadro por material com a taxa desenhada e "atende / abaixo da zona".

**Decisões.** (1) Sub-região vive no payload: é desenho do projetista, como a divisa e a faixa restrita. (2) Sem sub-região a permeabilidade fica NÃO AVALIADA em vez de zero — a ausência de desenho não é afirmação. (3) Sobreposição é aviso, não erro, e a área não é descontada — quem sobrepôs decide. (4) Sem toposuperfície: o polígono é plano; cortes/aterros continuam na terraplenagem.

**Prova.** *No app real* (estudo "Planta 14/09/2026", escritas bloqueadas 16): Terreno › Sub-região (barra "Grama") → 4 cliques + fechamento → painel "Sub-região do terreno · 17,41 m² · 4 vértices · permeável"; material trocado para intertravado e segunda sub-região; canvas com grama (pontos) e piso intertravado (grade), nome e área (captura); Dados do lote: "Grama · permeável 17,41 m² · Piso intertravado 6,45 m² · Permeável 17,41 m² · impermeável 6,45 m² · taxa de permeabilidade desenhada 8,8 %"; 0 erros de página. Testes: `blueprintSubRegioes.test.ts` (2: catálogo/comandos/invariantes/RemoveLevel; quadro, variáveis e regra CONFORME/VIOLADA/NÃO AVALIADA, medida de orçamento, canônico ida e volta), editor P2.19 (ferramenta e barra, Dados do lote com quadro e taxa, painel isolado), goldens recapturadas; suíte 419 arquivos / 4922 testes; tsc, check-ui e build OK; bundle regenerado e `planta-api` publicada (`plantaApi.test.ts` 3/3).

### P2.20 — Cortina de vidro e brises (21/09/2026) · backlog P2 · kernel 0.53.0 → 0.54.0 · quant-1.14.0 → 1.15.0

**O que entrou.** A pele da fachada, sem entidade nova: a parede pode SER **cortina de vidro** e/ou LEVAR **brise**.

- **Kernel**: `Wall.cortina {moduloMm ≥ 300, montanteMm ≥ 20 e < módulo, painel VIDRO|ACM|POLICARBONATO}` e `Wall.brise {orientacao HORIZONTAL|VERTICAL, laminaMm ≥ 20, passoMm ≥ lâmina, afastamentoMm ≥ 0, lado ESQUERDA|DIREITA (relativo a a→b, como o alinhamento e as camadas)}`; comandos `SetWallCortina`/`SetWallBrise` (`null` tira); invariantes `BAD_CURTAIN`/`BAD_BRISE`; `MergeWalls` recusa pele diferente (`MERGE_SKIN_MISMATCH`); cópias de grupo e de pavimento tipo levam a pele. Canônico emite as duas chaves só quando existem — **bump 0.54.0**, ritual dos goldens no cabeçalho (398 testes com a string velha; depois só os 6 hashes).
- **Quantitativo (quant-1.15.0)**: a parede-cortina fica com `volumeM3 = 0` e sem camadas (não há alvenaria a comprar) e ganha `cortina {painel, areaM2 (face líquida), paineis = ⌈comprimento ÷ módulo⌉, montantesM = (painéis+1) × altura + 2 × comprimento}`; o brise dá `brise {areaM2 = comprimento × altura, laminas, comprimentoLaminasM}`; totais `areaCortinaM2`, `comprimentoMontantesM`, `porCortina`, `areaBriseM2`, `laminasDeBrise`. **Orçamento**: medidas `AREA_CORTINA` (m², filtro pelo painel), `COMPRIMENTO_MONTANTE` (m) e `AREA_BRISE` (m²).
- **Saídas**: IFC emite **`IfcCurtainWall`** para a cortina (mesma geometria) e declara que o brise não sai; 3D pinta a cortina como vidro translúcido em qualquer estilo; canvas marca os montantes a cada módulo (azul) e o brise como faixa afastada da face com as lâminas (terracota).
- **UI**: seção **Fachada** no painel da parede — caixa "Cortina de vidro" (painel, módulo, montante) e "Brise" (orientação, face, lâmina, passo, afastamento), com padrões ao marcar (1200/60 vidro; horizontais 150/300/300 à direita).

**Decisões.** (1) Atributo da parede, não família nova: a cortina continua fechando ambiente, recebendo esquadria e cotas; só muda o que ela É no quantitativo, no IFC e no 3D. (2) A cortina zera a alvenaria; o brise não mexe na parede (é segunda pele). (3) Montantes e travessas numa conta simples e declarada; a espessura da parede é a profundidade do montante.
...
### P2.33 — Portas e janelas na importação DXF (21/09/2026) · pedido do dia, com print

**Pedido**: *"ao importar arquivos DXF o app deve reconhecer alguns símbolos como por exemplo o arco de porta e símbolo de janela"*. Decisões (AskUserQuestion): alturas por **hipóteses editáveis no painel** (porta 2,10 m; janela peitoril 1,00 m e altura 1,20 m; vão livre = pé-direito); buraco sem símbolo = **vão livre até 3 m**, acima ficam duas paredes.

**O que entrou**
- `utils/dxfLeitor.ts`: `ARC`/`CIRCLE` passam a ser LIDOS (`arcos`: centro, raio, ângulos) em vez de recusados — só `ELLIPSE`/`SPLINE` continuam nas recusas. Seção `BLOCKS` lida e `INSERT` **expandido** (escala → giro → translação menos o ponto-base, aninhado até 4 níveis, camada `0` herda a do INSERT, segmento/arco leva o nome do bloco; anônimos `*…` ignorados; bloco inexistente → recusa). `porCamada` conta arcos; `blocosExpandidos`; recusa não corta mais camada com espaço.
- `utils/emendaDeParedes.ts` (novo, compartilhado): `emendarColineares(paredes, { vaoMinMm, vaoMaxMm, compativeis?, classificar, fundir? })` — o miolo do `emendarVaosLivres` do COLLADA (P2.29), agora com o classificador injetado, a orientação das peças normalizada (peça ao contrário é virada e os offsets espelhados — defeito latente da P2.29) e a guarda **"nada no meio do buraco"**: duas portas com pilarete entre elas deixam três peças colineares, e o par das pontas emendava por cima do pilarete (um "vão" de 1,9 m engolindo as duas portas) — achado no arquivo real. `colladaParaKernel.emendarVaosLivres` virou uma chamada a ele; `AberturaLida` mora lá (com `hingeAtStart?`/`swingReversed?`).
- `utils/dxfParaKernel.ts` `aberturasDoDxf(paredes, leitura, mmPorUnidade, camadaDeParede, alturaDaParedeMm, hip)`: **porta** = arco de 60–120° com raio 500–1600 mm, centro numa ponta do buraco (a meia espessura + 150 mm) e a folha fechada terminando na outra ponta → `door` com largura do buraco, `hingeAtStart` pela ponta do centro e `swingReversed` pelo lado do ponto médio do arco (normal `n = (−uy, ux)` do kernel); porta de **duas folhas** = um arco em cada ponta com raio ≈ metade; arco em cima de parede **contínua** → porta com a largura do raio; porta no **canto** (10 das 35 do arquivo real: a dobradiça é a parede perpendicular, só há trecho colinear de um lado) → a parede é esticada até o canto e a porta fica na ponta — só quando a folha fechada termina exatamente na ponta da parede, o que distingue a parede da porta da parede perpendicular. **Janela** = ≥ 2 traços paralelos (±1°) dentro do buraco, na faixa da espessura (+50 mm), cobrindo ≥ 60 % — procurados em todas as camadas menos a de parede; nome de bloco `porta|door` / `janela|window|jan` vale como símbolo. **Vão livre** = buraco sem símbolo até `vaoLivreMaxMm`. **Tocos de batente** (peça mais larga que comprida com o centro dentro do corpo de outra parede — as duas linhas do batente pareadas como "parede") são descartados e contados. `HIPOTESES_ESQUADRIAS_PADRAO`, `ResumoDeEsquadrias { portas, janelas, vaos, arcosSemParede, tocosDeBatente }`.
- `PainelImportarDxf.tsx`: bloco **Esquadrias** (checkbox "Reconhecer portas (arco) e janelas (símbolo) em todas as camadas"; altura da porta, peitoril e altura da janela, vão livre até — `usePersistedState('blueprint:dxf-esquadrias')`); linha "N porta(s) · N janela(s) · N vão(s) livre(s) · arcos longe de parede · tocos descartados · blocos expandidos"; botão "Importar N + M"; lote `AddWall{uid}` + `AddOpening{wallUid, kind, offset, width, height ≤ pé-direito − peitoril, sill, hingeAtStart, swingReversed}` (molde da P2.29). Camadas listam "· N arcos". Sem bump de kernel.

**Decisões**
- Símbolo em qualquer camada, parede só na escolhida: no arquivo real o arco mora em `PORTAS` (38) e o vidro em `JANELAS` (458 traços); a camada de parede é excluída da busca de janela porque as faces dela não são vidro.
- O DXF é planta: alturas são hipóteses, nunca "lidas". A tela diz isso ao lado dos campos.
- Bloco entra (arquivo de terceiro tem esquadria em bloco), mas no arquivo real os 48 blocos expandidos são cota, vegetação e carro — nenhum é porta.
- Tolerância da dobradiça 150 mm além de meia espessura: medido, o centro fica a 175 mm do eixo numa parede de 150 (100 mm além da face) em várias portas.

**Prova**
- `npx tsc --noEmit` ok · check-ui ok · `check-xss-sinks.sh` ok · suíte cheia **432 arquivos / 5020 testes** · `npm run build` ok.
- App real (escritas bloqueadas: 15, 0 erros), Planta 14/09: `casa-print.dxf` (dois cômodos gerados como o print: faces em PAREDE param no batente, arcos em PORTAS, duas linhas em JANELAS, um arco solto em ARQ-LAYOUT) → camada PAREDE e milímetro sugeridos → **"5 paredes · 32,00 m" / "2 porta(s) · 3 janela(s) · 0 vão(s) livre(s) · 1 arco(s) de porta longe de parede, ignorado(s)"** → altura da porta 2 200 → "Importar 5 + 5" → 14 → 19 paredes; no canvas as duas portas com o arco para o lado certo (a interna abre para o cômodo B) e as três janelas no vão. **DXF real da empresa** (8,3 MB, metro): camada PAREDE → **"332 paredes" / "34 porta(s) · 38 janela(s) · 89 vão(s) livre(s) · 38 arco(s) longe de parede (os 32 do SELO) · 66 toco(s) de batente descartado(s) · 48 bloco(s) expandido(s)"**, "Importar 332 + 161", recusas: nenhuma (antes: 38 ARC em PORTAS, 328 INSERT). Das 35 folhas de porta desenhadas (38 arcos − 3 que não são folha: 39°, 0°, raio 243 mm), 34 viraram porta, todas de 0,80 m — a parede do corredor sai com seis portas em sequência, a cara do print.
- Testes: `dxfLeitor.test.ts` (15: ARC/CIRCLE lidos; SPLINE/ELLIPSE recusadas; bloco com escala 2 e giro 90°, espelho invertendo o sentido do arco, bloco inexistente, `*D12` ignorado, bloco dentro de bloco), `dxfEsquadrias.test.ts` (11: porta/janela/vão/3,5 m separado, símbolos desligados, hipóteses e pé-direito baixo, teto do vão livre, lado e dobradiça pelo arco, parede contínua, pilarete entre duas portas, porta dupla, porta no canto, blocos PORTA-80/JAN-120, lote aceito pelo kernel), `dxfParaKernel.test.ts` (arquivo real: ≥ 30 portas, ≥ 30 janelas, todas ~0,80 m, corredor com ≥ 5), `PainelImportarDxf.test.tsx` (11: resumo, lote com uid/wallUid e hipóteses, persistência, símbolos desligados), `colladaParaKernel.test.ts` verde após a extração.

### P2.34 — Leitura precisa do DXF: fresta, encosto, canto, filtro de espessuras e relatório (22/09/2026) · a partir do print da P2.33

**Pedido**: depois da P2.33 o usuário abriu o arquivo real importado — "são muitos pontos pendentes" — e perguntou como ler com mais precisão, cogitando padronizar a simbologia. Diagnóstico pelo print: os pontos são **pontas de parede soltas**, em pares ao longo de paredes retas (cruzamento em T de um desenho de faces), nos cantos com recuo, e por consequência só 2 ambientes fechavam. Decisão: **P2.34** corrige a leitura sem exigir nada do desenho de origem; o **Padrão ÒPURA de desenho** (template com eixo por espessura e blocos com atributos) fica como P2.35.

**Régua** (kernel, arquivo real da empresa, camada PAREDE, todas as espessuras): P2.33 deixava **328 pontas soltas** (metade de todas as pontas) e 106 "ambientes" de área zero; P2.34 deixa **239 pontas soltas e 123 ambientes**, os maiores com 57, 52, 45 m² — cômodos de verdade. Com o filtro "só as principais" (100/150/200/300/400): 219 paredes, 135 pontas soltas.

**O que entrou**
- `utils/emendaDeParedes.ts`: **fresta** (`frestaMaxMm`) — buraco menor que o vão mínimo emenda SEM abertura (o T: a face da parede que passa é interrompida pela que chega, 10–25 cm); **sobreposição pequena** (`largura` negativa até `frestaMaxMm`, nunca mais que meia peça) emenda pela união; o classificador pode devolver uma LISTA de aberturas com offset relativo ao buraco (porta de duas folhas → duas portas de meia largura, dobradiças nas pontas, mesmo lado). COLLADA continua com fresta 0 (comportamento da P2.29 intacto).
- `utils/dxfParaKernel.ts`: depois da emenda e dos tocos, **`encostarNasFaces`** (o do IFC) leva ao eixo a ponta que parou na face da parede que cruza, deslocando os offsets das aberturas; **`fecharCantos`** — ponta solta a até 300 mm de outra ponta de parede, não paralela, vai (com a outra) ao cruzamento dos eixos se nenhuma anda mais que 300 mm (a mitragem só estica; o encosto só atende quem cruza pelo meio; sobrava o L em que uma peça avança e a outra para curta). `ResumoDeEsquadrias += encostadas, pontasSoltas, cantosFechados`; `HIPOTESES_ESQUADRIAS_PADRAO.frestaMaxMm = 299`.
- `PainelImportarDxf.tsx`: **filtro de espessuras** (chips "150 · 137 · 894 m" ligáveis; "Só as principais" = ≥ 4 % do comprimento; "Todas") aplicado antes do reconhecimento; **Relatório de leitura** (paredes, esquadrias, "N ponta(s) de parede vão ficar soltas · N encostada(s) · N canto(s) fechado(s)", e a linha do que ficou fora: filtro, sobrepostas, tocos, arcos longe, blocos).

**Decisões**
- Nenhuma regra nova "inventa" geometria além do que a P2.26/IFC já aceitava: fresta e sobreposição só entre peças colineares da mesma espessura sem nada no meio; encosto só dentro do corpo da outra; canto só entre pontas a 30 cm — vão de porta (≥ 80 cm) nunca é alcançado.
- Pendência ligada à peça não ganhou tela nova: o app já marca ponta solta no canvas (círculo âmbar), tem a ferramenta Juntar e sugere "Vão N" entre pontas próximas; o que faltava era o número ANTES de importar, e isso o relatório dá.
- O que resta solto no arquivo real é, em boa parte, desenho de prancha (cortes, elevações, grade de vaga, muro de 400) e pares de linhas que não são parede — o filtro de espessura é o controle, e o Padrão ÒPURA (P2.35) é a solução de raiz.

**Prova**
- `npx tsc --noEmit` ok · check-ui ok · `check-xss-sinks.sh` ok · suíte cheia **432 arquivos / 5026 testes** · `npm run build` ok.
- App real (escritas bloqueadas: 15, 0 erros), Planta 14/09: casa do print → relatório "5 paredes · 2 porta(s) · 3 janela(s) · **Todas as pontas de parede encostam em outra**". **DXF real da empresa**, camada PAREDE, todas as espessuras: "299 paredes · 34 porta(s) · 38 janela(s) · 89 vão(s) · **220 ponta(s) de parede vão ficar soltas · 47 encostada(s) · 30 canto(s) fechado(s)** · 32 sobrepostas · 65 tocos · 38 arcos longe · 48 blocos"; 34 espessuras oferecidas → **Só as principais** (100, 150, 200, 300) → "219 paredes · 32 porta(s) · 38 janela(s) · 54 vão(s) · 135 ponta(s) soltas · 138 parede(s) fora pelo filtro" → "Importar 219 + 124" → **14 → 233 paredes, 4 → 117 ambientes**; no canvas as duas plantas da casa aparecem com os cômodos fechados (sombreados), e as pontas soltas restantes estão nas peças de prancha (cortes, grade, muros).
- Testes: `dxfEsquadrias.test.ts` (15: + fresta em T com encosto da parede que chega, sobreposição pequena vira uma parede, canto em L fecha no cruzamento dos eixos, porta dupla em duas folhas; a porta no canto agora termina no eixo da perpendicular), `dxfParaKernel.test.ts` (arquivo real: ≥ 30 encostadas, ≥ 5 cantos, < 250 pontas soltas, > 110 ambientes, maior entre 40 e 80 m²), `PainelImportarDxf.test.tsx` (13: chips de espessura com contagem, desmarcar tira a parede do lote, "Só as principais"/"Todas", relatório com pontas soltas), `colladaParaKernel.test.ts` verde.

### P2.35 — Padrão ÒPURA de desenho para importação (22/09/2026) · a solução de raiz para a leitura do DXF

**Pedido**: "P2.35" — o padrão proposto depois do print da P2.33: em vez de reconhecer, LER. O DXF comum obriga a adivinhar (que par de linhas é parede, que arco é porta, que altura tem a janela); o padrão elimina cada adivinhação com uma convenção pequena que o projetista segue num template.

**O padrão (v1.0)**
- **Parede** = um traço no EIXO, na camada `OPURA-PAREDE-<espessura em mm>` (`OPURA-PAREDE-150`). Espessura pelo nome da camada; sem pareamento.
- **Esquadria** = bloco `OPURA-PORTA` / `OPURA-JANELA` / `OPURA-CORRER` / `OPURA-VAO` com o ponto-base sobre o eixo, na ombreira da dobradiça; +X do bloco para a outra ombreira, +Y para o lado em que abre (espelhar inverte). Atributos `LARGURA`, `ALTURA`, `PEITORIL` (mm) e `TIPO` (nome no projeto) são lidos; o que faltar cai nas hipóteses do painel.
- **Ambiente** = `TEXT` com o nome, dentro do cômodo, na camada `OPURA-AMBIENTE`.
- Unidade milímetro; um arquivo por pavimento; desenho perto da origem. As regras vão escritas no próprio template (camada `OPURA-NOTAS`) e no painel ("Ver as regras").

**O que entrou**
- `utils/dxfLeitor.ts`: `LeituraDxf.insercoes` (INSERT do nível de cima com `atributos` — os `ATTRIB` que seguem o INSERT até o `SEQEND`, por TAG) e `LeituraDxf.textos` (TEXT/MTEXT, com a formatação do MTEXT limpa). De passagem: a chave de recusa tinha um NUL literal no fonte (o arquivo aparecia como binário no git) — virou `\n` escrito.
- `utils/dxfPadraoOpura.ts` (novo): `gerarTemplateOpura()` (DXF R12 com as camadas, os 4 blocos com `ATTDEF`, um cômodo de exemplo com porta P1, janela J1 e o texto SALA, e as regras), `temPadraoOpura`, `lerPadraoOpura(leitura, alturaParede, hip)` → paredes por `paredesDeEixos` por camada, esquadrias pelo bloco (parede hospedeira = eixo a até meia espessura + 50 mm com o +X do bloco alinhado; `hingeAtStart` pelo sentido do +X, `swingReversed` pelo +Y contra a normal do kernel; largura maior que a parede ou sobreposta é relatada, não entra), ambientes pelos textos; junções pelo mesmo `acabarJuncoes` da P2.34 (extraído de `aberturasDoDxf`).
- Kernel (sem bump — etiquetas já são canônicas): comando **`PlaceSpaceLabel { levelId, at, name }`** — a etiqueta nasce no ponto, no mesmo lote das paredes; se já há ambiente com etiqueta ali, renomeia; se as paredes não fecharam, fica no ponto e nomeia o ambiente quando ele fechar.
- `PainelImportarDxf.tsx`: bloco **Padrão ÒPURA** antes de escolher o arquivo ("Baixar template (.dxf)", "Ver as regras"); arquivo detectado → aviso "Arquivo no Padrão ÒPURA — lido, não reconhecido" com o resumo (paredes por espessura, portas, janelas, de correr, vãos, ambientes, blocos longe/que não couberam), checkbox para ler como DXF comum; camada, unidade, caminho e filtro de espessuras somem (o padrão já respondeu); lote com `esquadria.nome` = `TIPO`, `sliding` para `OPURA-CORRER` e `PlaceSpaceLabel` por texto (deslocado pela ancoragem).

**Decisões**
- Uma versão do padrão (`VERSAO_DO_PADRAO = '1.0'`) escrita no template e no nome do arquivo: mudança de convenção no futuro é versão nova, não surpresa.
- Bloco fora de parede ou largo demais é RELATADO e não entra — o padrão é para ser exato; entrar "encostado" esconderia o erro do desenho.
- Pavimento não entra no padrão (um arquivo por pavimento, importado no pavimento ativo) — a criação de pavimento pela cota já existe para IFC/COLLADA (P2.32) e o DXF de planta não tem cota.

**Prova**
- `npx tsc --noEmit` ok · check-ui ok · `check-xss-sinks.sh` ok · suíte cheia **433 arquivos / 5034 testes** · `npm run build` ok.
- App real (escritas bloqueadas: 15, 0 erros), Planta 14/09: Inserir › Do DXF/DWG → **"Baixar template (.dxf)"** baixa `padrao-opura-v1.0.dxf` (4,8 kB; 7 regras listadas) → o template baixado editado como um projetista faria (segundo cômodo em `OPURA-PAREDE-150`, `OPURA-CORRER` de 1500 girado 90° na divisória, `OPURA-VAO` de 1200, texto COZINHA) → painel: **"Arquivo no Padrão ÒPURA — lido, não reconhecido · 7 parede(s) de 150 mm · 1 porta(s) · 1 janela(s) · 1 de correr · 1 vão(s) · 2 ambiente(s)"**, camada/unidade ocultas, "Todas as pontas de parede encostam em outra", "Importar 7 + 6" → 14 → 21 paredes, 4 → 6 ambientes, **SALA e COZINHA nomeados** no mesmo passo; no canvas a porta P1 com o arco para dentro, a janela, a de correr na divisória e o vão.
- Testes: `dxfPadraoOpura.test.ts` (5: o template volta pelo leitor com blocos/ATTRIB/TEXT; paredes, porta e janela pelos atributos com dobradiça e lado; atributo ausente cai na hipótese, largura demais e bloco longe relatados; girado 180° e espelhado em Y; lote com `PlaceSpaceLabel` nomeia o ambiente, nome vazio recusado, ponto fora fica esperando, renomear substitui), `PainelImportarDxf.test.tsx` (16: detecção, resumo, perguntas ocultas, lote com esquadria e etiqueta, voltar ao caminho comum, template e regras antes do arquivo), `dxfLeitor`/`dxfEsquadrias`/`colladaParaKernel` verdes.

### P2.36 — O DXF/DWG original como planta de fundo, já aferido (22/09/2026) · pedido do dia

**Pedido**: *"seria interessante ao importar uma planta DWG/DXF com a mesma funcionalidade que existe ao importar uma planta em PDF que é a visualização da planta original por baixo dos componentes gerados. Assim podemos comparar o desenho e fazer as correções caso necessário"*.

**O que entrou**
- `utils/dxfParaFundo.ts` (novo): `planejarFundo(leitura, { mmPorUnidade, dx, dy, camadas?, camadaDestaque? })` — caixa do desenho em mm (com a ancoragem das paredes), resolução pelo lado maior (`LADO_MAX_PX` 4096, piso 1 mm/px), margem de 8 px e o `Underlay` exato (pixel (0,0) = canto superior esquerdo; Y invertido uma vez só); `desenharFundo(ctx, …)` — todas as camadas em cinza-azulado e a camada de parede escolhida por último, mais escura; **traço em milímetros do modelo** (20 mm; 30 na de destaque), não em pixels — a primeira prova mostrou que 1 px numa imagem a 3,7 mm/px vira 0,3 px na tela enquadrada, invisível; `rasterizarDxf` → PNG transparente (a grade continua visível por baixo).
- `hooks/useBlueprintUnderlay.ts`: `importarRaster(blob, nome, underlay, larguraPx)` — sobe o PNG e grava a prancha **já aferida**: a aferição gravada é verdadeira (largura da imagem em px contra a largura do desenho em mm, ambas do arquivo), não sintética — é o que impede a tela de pedir aferição a um fundo que já a tem. Entra como mais uma prancha do pavimento (não substitui o PDF que já estiver lá).
- `BlueprintEditor.tsx` → `PainelImportarDxf` recebe `onFundo` e `fundoAtivo`. Painel: bloco **"Guardar o desenho original como planta de fundo, para comparar"** (persistido, padrão ligado) com o plano do raster ("4096 × 2189 px · 3,7 mm/px · camada PAREDE em destaque"); ao importar, o fundo sobe ANTES das paredes — se falhar (navegador sem canvas, upload recusado), as paredes entram do mesmo jeito e o aviso fica no painel. Vale para o caminho comum, para o Padrão ÒPURA e para o DWG convertido.

**Decisões**
- Todas as camadas no fundo, não só a de parede: é justamente o que a leitura NÃO trouxe (cota, mobiliário, texto) que ajuda a conferir; a camada de parede sai em destaque para o olho comparar com a parede gerada.
- PNG transparente e traço fino: fundo é referência, não desenho.
- Sem toggle de visibilidade novo: a prancha entra no mesmo lugar do PDF (Inserir › Referência: prancha ativa, opacidade, remover).

**Prova**
- `npx tsc --noEmit` ok · check-ui ok · `check-xss-sinks.sh` ok · suíte cheia **434 arquivos / 5041 testes** · `npm run build` ok.
