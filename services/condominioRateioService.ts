...
// O resolvedor do documento de origem mora em `_shared` porque tem DOIS
// leitores: esta camada (aba Despesas, relatório do síndico) e a Edge Function
// que assina os comprovantes para o portal do condômino. Ver o cabeçalho de lá.
import {
    dadosDoBoleto as dadosDoBoletoCom,
    dadosDaNfe as dadosDaNfeCom,
    dadosDeContrato as dadosDeContratoCom,
    resolverOrigens as resolverOrigensCom,
    type DocumentoDeOrigem,
    type OrigemResolvida,
} from '../supabase/functions/_shared/origemDoLancamento';

export { uuidDaReferencia, numeroDaChaveNfe } from '../supabase/functions/_shared/origemDoLancamento';
export type { DocumentoDeOrigem, OrigemResolvida } from '../supabase/functions/_shared/origemDoLancamento';
...
export const condominioRateioService = {
    ...
    // As quatro abaixo delegam ao resolvedor compartilhado com a sessão do
    // usuário — a RLS de cada tabela de origem continua valendo aqui.
    dadosDoBoleto(boletoIds: string[]): Promise<Map<string, DadosDoBoleto>> {
        return dadosDoBoletoCom(supabase, boletoIds);
    },

    dadosDaNfe(nfeIds: string[]): Promise<Map<string, OrigemResolvida>> {
        return dadosDaNfeCom(supabase, nfeIds);
    },

    dadosDeContrato(refs: string[]): Promise<Map<string, OrigemResolvida>> {
        return dadosDeContratoCom(supabase, refs);
    },

    resolverOrigens(
        linhas: { id: string; source_system?: string | null; reference_id?: string | null }[],
    ): Promise<Map<string, OrigemResolvida>> {
        return resolverOrigensCom(supabase, linhas);
    },
