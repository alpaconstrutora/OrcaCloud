...
export const rentalExecutiveService = {
    ...
    async load(
        ...
        try {
            let q = supabase
                .from('contracts')
                .select('id, status, start_date, end_date, current_value, original_value, parent_contract_id, renewal_seq')
                .eq('domain', 'LOCACAO');
            ...
            const rows = (contratos || []) as {
                id: string; status?: string | null;
                end_date?: string | null;
                current_value?: number | null; original_value?: number | null;
                parent_contract_id?: string | null; renewal_seq?: number | null;
            }[];
            ...
            const recebiveis: { amount: number; due_date?: string | null; settled: boolean }[] = [];
            ...
            for (const lote of chunk(ids, IN_CHUNK)) {
                ...
                for (const r of (data || []) as { amount: number; due_date?: string | null; status?: string }[]) {
                    recebiveis.push({
