-- Adiciona rich_contract_id a services_opportunities e atualiza trigger de conversão.
-- Contexto: services_opportunities.converted_contract_id aponta para services_contracts
-- (modelo fino). Esta migration adiciona rich_contract_id apontando para contracts
-- (modelo rico com medição, aditivo, retenção, sync financeiro).
-- Trigger atualizado para criar ambos na conversão 'won'.
...
-- 2. Trigger de conversão: cria services_contract (fino) + contract (rico) ao marcar 'won'
...
BEGIN
    ...
    -- 1) Caso engenharia + projeto vinculado: promove ORCAMENTO → OBRA
    ...
    -- 2) Caso simples: cria projeto novo
    ...
    -- 3) Cria o contrato FINO (services_contracts) — backward compat
    ...
    -- 4) Cria o contrato RICO (contracts) — nova fonte de verdade
