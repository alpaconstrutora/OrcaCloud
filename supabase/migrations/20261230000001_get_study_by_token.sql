-- RPC para carregar o estudo de viabilidade do IMOVIB de forma segura por token público do investidor (anon)
-- Date: 2026-06-29
...
BEGIN
    ...
    -- 2. Garante que existe uma oportunidade ativa/publicada com esse study_id pertencente à organização
    SELECT * INTO v_opp
    FROM public.investor_opportunities
    WHERE imovib_study_id = p_study_id AND organization_id = v_org_id;
    IF NOT FOUND THEN RETURN '{"valid":false,"error":"forbidden"}'::jsonb; END IF;

    -- 3. Reconstrói o estudo em JSON
