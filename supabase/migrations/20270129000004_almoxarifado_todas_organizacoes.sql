-- ============================================================
-- Almoxarifado — fn_net_position / fn_stock_summary suportam
-- "Todas as Organizações" (p_organization_id NULL)
-- OrçaCloud SaaS · Migration 20270129000004
...
BEGIN
  ...
  v_targets := CASE WHEN p_organization_id IS NULL THEN v_org_ids ELSE ARRAY[p_organization_id] END;
    ...
    movements_agg AS (
        ...
        WHERE sm.organization_id = ANY(v_targets)
    ...
    reservations_agg AS (
        ...
        FROM public.stock_reservations sr
        WHERE sr.organization_id = ANY(v_targets) AND sr.status = 'active'
        GROUP BY sr.warehouse_id, sr.input_code
    )
...
-- FIM: 20270129000004_almoxarifado_todas_organizacoes.sql
