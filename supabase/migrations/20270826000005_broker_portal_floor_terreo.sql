-- ==========================================================================
-- Pavimento TÉRREO (0) sumia no link do Portal do Corretor
-- Date: 2026-07-26
-- ==========================================================================
-- CONTEXTO
-- As duas RPCs do portal montavam o pavimento com
--     COALESCE(NULLIF(p.floor, 0), NULLIF((p.specs->>'floor')::int, 0))
-- O NULLIF(p.floor, 0) existia porque dormitórios/vagas/banheiros ficam em 0 na
-- coluna quando o dado real está em `specs` — mas PAVIMENTO é diferente dos
-- outros três: 0 é o TÉRREO, um valor legítimo. O resultado era o térreo virar
-- NULL e aparecer como "—" no link, enquanto a visão do app (que lê a coluna
-- direto) mostrava 0. No Garden Cambuhy isso pega as unidades 002 a 008.
--
--     COALESCE(p.floor, NULLIF((p.specs->>'floor')::int, 0))
-- mantém o fallback para specs quando a coluna é NULL, sem descartar o zero.
--
-- Os corpos abaixo são cópia FIEL de 20270826000004 (switch "Exibir Preço"),
-- que é a versão mais recente destas funções — só a linha do 'floor' muda.
-- Aplicar 20270826000004 ANTES desta: as funções referenciam a coluna
-- show_price_to_broker, criada lá.
--
