...
--
-- O número de um documento é gerado SÓ na criação (ContractModal:420) e nunca
-- muda ao editar, de propósito: ele é a identidade do documento e está no papel
...
--      `signed_contract_url` preenchido, ou `status` em Assinado/Ativo/Concluído).
--
...
COMMENT ON TABLE public.contract_number_history IS
    'Trilha de "Regerar número" de contrato. Sem ela, quem procurar pelo número antigo não acha nada.';
...
-- Sem policy de INSERT/UPDATE/DELETE de propósito: só a RPC abaixo escreve,
-- e ela é SECURITY DEFINER. Trilha que o próprio usuário pode reescrever não
-- é trilha.
