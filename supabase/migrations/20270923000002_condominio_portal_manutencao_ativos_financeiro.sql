-- 20270923000002 — Portal do Cliente › Condomínio: manutenção, ativos e rateio
-- Plano: docs/planos/2026-09-23-condominio-portal-manutencao-ativos-financeiro.md
--
-- POR QUE ESTA MIGRATION EXISTE
--
-- Das 8 abas de Comercial › Condomínios, três nunca tiveram saída para o portal:
-- Ativos, Manutenção e Financeiro. O condômino via a unidade, os avisos e os
-- documentos — e nada do que a administração faz com o prédio nem de como a
-- cota dele é calculada.
--
-- O payload é UM só (`fn_condominio_payload_for_client`), então as duas portas
-- do portal — link (`client_portal_get_condominio`) e cliente logado
-- (`..._for_client`) — ganham os três blocos de uma vez, sem drift.
--
-- RECORTE, decidido pelo usuário em 23/09/2026:
--   • Financeiro: rateio COMPLETO, incluindo a cota das outras unidades
--     ("transparência de assembleia"). Não é vazamento acidental: é a escolha
--     explícita entre três opções apresentadas, e o que sai é unidade + nome de
--     quem é cobrado + valor. Documento e contato de terceiro continuam fora,
--     como já valia para `ocupacoes`.
--   • Rateio FECHADO e RASCUNHO, com o status junto para a tela rotular o
--     rascunho como "prévia — pode mudar". CANCELADO não entra: são 4 dos 5
--     rateios da base, e cancelado é decisão desfeita, não "em aberto".
--     Mostrar só FECHADO faria a seção nascer vazia (há 0 fechados hoje).
--   • Manutenção e Ativos: plano + ordens + equipamentos, leitura.
--
-- Nada de policy nova: a função é SECURITY DEFINER e já tinha a autorização nas
-- duas RPCs que a chamam (token válido, ou membro da organização / o próprio
-- cliente pelo e-mail). Ver REGRA OBRIGATÓRIA #7.
...
AS $$
    ...
    SELECT jsonb_build_object(
        ...
        -- Sem `purchase_value`, `supplier_id` nem `serial_number`: preço de
        -- compra e fornecedor são da administração, e número de série só serve
        -- a quem opera o equipamento. Garantia entra porque é o que o condômino
        -- tem interesse legítimo em cobrar.
        'ativos', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
                'id', a.id,
                'nome', a.name,
