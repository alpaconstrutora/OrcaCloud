...
--
-- O kernel ganhou a família `stairs`: o percurso que vence um desnível, com
-- tipo (escada ou rampa), largura e o ALVO de espelho. O número de degraus não
-- viaja no payload — é derivado do desnível entre pavimentos a cada leitura.
-- Do lado do banco mudam as duas coisas de sempre (molde:
-- `aplicar_20270919000007`, o corte):
--
--   1. `blueprint_objects.object_type` precisa aceitar 'STAIR'.
--   2. `fn_blueprint_publish_snapshot` precisa explodir `payload->'stairs'`,
--      com o `element_uid` lido de `identity.stairs`.
--
-- NENHUMA tabela nova e NENHUMA coluna nova.
--
-- ⚠️ O texto da função abaixo veio do ARQUIVO da migration anterior, e NÃO de
-- `pg_get_functiondef` no banco (que corrompe os acentos dos comentários).
--
...
ALTER TABLE public.blueprint_objects
    ...
    CHECK (object_type IN ('WALL','OPENING','BOUNDARY','SPACE','STRUCTURAL','ROOF','SECTION','STAIR'));

-- ─── 2. A publicação explode também as escadas ──────────────────────────────
--
-- `CREATE OR REPLACE` da função inteira: plpgsql não tem como emendar um laço
-- no meio de um corpo já criado. O texto abaixo é o de `aplicar_20270919000007`
-- com UM bloco novo, o das escadas, logo depois do dos cortes.
SET lock_timeout = '5s';
...
BEGIN
    ...
    -- ESCADA E RAMPA (kernel 0.14.0). `COALESCE` pela MESMA razão das outras
    -- famílias omitidas quando vazias: a chave `stairs` não aparece no payload
    -- de um desenho sem circulação vertical, para que o hash das plantas
    -- antigas não mude.
    --
    -- `level_index` PREENCHIDO, ao contrário do corte: a escada pertence ao
    -- pavimento de PARTIDA (é dele que ela sobe, e é com ele que ela some em
    -- `RemoveLevel`). O pavimento de chegada NÃO é gravado — é derivado da cota
    -- e mudaria sozinho se alguém acrescentasse um andar no meio.
    --
    -- `area_mm2` e `length_mm` ficam NULL, pelo motivo já escrito na água e no
    -- corte: essas colunas hoje significam "área de ambiente" e "metro linear
    -- de parede", e quem as soma está somando isso. A pegada e o comprimento
    -- da escada existem, mas entrariam numa soma que não é deles. Quem precisa
    -- lê `props`, que carrega o item canônico inteiro — e o número de degraus
    -- NÃO está lá de propósito: ele é derivado do desnível, e gravado ficaria
    -- obsoleto no primeiro ajuste de cota.
    FOR v_item, v_idx IN
        SELECT value, ord - 1 FROM jsonb_array_elements(COALESCE(p_payload->'stairs','[]'::jsonb))
          WITH ORDINALITY AS t(value, ord)
    LOOP
        INSERT INTO public.blueprint_objects
            (snapshot_id, organization_id, object_type, object_index, level_index, props, element_uid)
        VALUES (
            v_snapshot_id, v_org_id, 'STAIR', v_idx, (v_item->>'level')::INTEGER, v_item,
            p_payload #>> ARRAY['identity','stairs', v_idx::TEXT]
        );
    END LOOP;
...
-- SELECT position('stairs' in pg_get_functiondef('public.fn_blueprint_publish_snapshot'::regproc)) > 0;
