-- Condomínio no Portal do Cliente: a despesa do rateio passa a levar o
-- FORNECEDOR, para o relatório de rateio (PDF) poder nomeá-lo.
--
-- Por que dois campos crus e não um nome pronto: a regra de qual nome usar
-- (cadastrado antes do texto cru, e o cru PODADO de CNPJ/endereço/publicidade)
-- é `utils/despesaCondominio.rotuloDeFornecedor`, em TypeScript, e é a mesma
-- que o admin usa. Decidir aqui criaria uma segunda regra, e o mesmo
-- fornecedor sairia com dois nomes no mesmo documento.
--
-- Medido em 25/09/2026: as 38 despesas de rateio da base acham o lançamento de
-- origem, e todas as 38 têm fornecedor cadastrado.
--
-- Esta migration REESCREVE `fn_condominio_payload_for_client` a partir do
-- arquivo da migration vigente (aplicar_20270924000050), mudando SÓ o
-- subselect de despesas. Conferido antes: o corpo vivo no banco e o do
-- repositório são idênticos nesse trecho.
    ...
    SELECT jsonb_build_object(
        ...
        'rateios', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
                ...
                -- O FORNECEDOR de cada despesa vai em DOIS campos crus, e não
                -- num nome já escolhido: quem decide qual usar é
                -- `utils/despesaCondominio.rotuloDeFornecedor`, a mesma função
                -- que o admin chama. Escolher aqui criaria uma segunda regra em
                -- SQL — e o `podarRuidoDeBoleto` (que corta CNPJ, endereço e
                -- publicidade do bloco de OCR) vive em TypeScript.
                --
                -- `condominio_rateio_despesas` não guarda o fornecedor: o
                -- snapshot é descrição + valor, e o nome vem do lançamento de
                -- origem por `transaction_id`. LEFT JOIN porque despesa sem
                -- lançamento (rateio montado à mão) continua na lista.
                'despesas', COALESCE((
                    SELECT jsonb_agg(jsonb_build_object(
                               'id', d2.id, 'descricao', d2.descricao, 'valor', d2.valor,
                               'fornecedorCadastrado', s2.name,
                               'fornecedorCru', COALESCE(t2.party_name, t2.entity_name))
                           ORDER BY d2.valor DESC)
                      FROM public.condominio_rateio_despesas d2
                      LEFT JOIN public.internal_transactions t2 ON t2.id = d2.transaction_id
                      LEFT JOIN public.suppliers s2 ON s2.id = t2.supplier_id
